...
// Function to get or create Analytics instance
function getAnalytics() {
  const segmentWriteKey = process.env.SEGMENT_WRITE_KEY;

  if (!segmentWriteKey) {
    throw new Error('SEGMENT_WRITE_KEY environment variable is not set');
  }
...
// Fetch conversation attributes from Twilio (contains application_id, etc.)
async function getConversationAttributes(conversationSid) {
  if (!conversationSid) {
    return null;
  }
  ...
  try {
    ...
    console.log('Fetched conversation attributes:', attributes);
    return attributes;
  } catch (error) {
    console.error('Error fetching conversation attributes:', error);
    return null;
  }
}
...
async function trackRcsMessageReceived(data, analytics) {
  ...
  try {
    await analytics.track({
      ...
      properties: {
        ...
        // Include conversation attributes (application_id, etc.)
        application_id: data.conversationAttributes?.application_id || null,
        candidate_name: data.conversationAttributes?.candidate_name || null,
        job_applied: data.conversationAttributes?.job_applied || null,
        ...data.conversationAttributes, // Include all other attributes
        ...timestampData,
      },
      timestamp: new Date(),
    });

    console.log('Segment event queued: RCS Message Received', {
      anonymousId: data.from,
...
async function trackRcsCardInteraction(data, analytics) {
  const timestampData = getTimestampData();
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 4. Fetch conversation attributes (contains application_id, etc.)
    const conversationAttributes = await getConversationAttributes(ConversationSid);

    // 5. Detect interaction type
    const interactionType = detectInteractionType(req.body);

    // 6. Track appropriate event in Segment
    const eventData = {
      ...
      interactionType,
      // Include conversation attributes (application_id, name, job_applied, etc.)
      conversationAttributes,
    };

    // 7. Create Analytics instance for this request
    const analytics = getAnalytics();

    console.log('About to track event with data:', {
      interactionType: interactionType.type,
      from: eventData.from,
      messageSid: eventData.messageSid,
      application_id: conversationAttributes?.application_id,
    });

    if (interactionType.type === 'text_reply') {
      // Standard inbound message
      await trackRcsMessageReceived(eventData, analytics);
    } else {
      // Rich card interaction (button, list, location)
      await trackRcsCardInteraction(eventData, analytics);
    }

    // 8. Flush analytics to ensure events are sent before function exits
    console.log('About to flush analytics...');
    try {
      await analytics.flush();
      console.log('Analytics flushed successfully');
    } catch (flushError) {
      console.error('Error flushing analytics:', flushError);
      throw flushError;
    }

    // 9. Return empty TwiML response (no auto-reply)
